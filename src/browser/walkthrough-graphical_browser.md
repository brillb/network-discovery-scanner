...
2. **Flask Application ([graphical_browser.py](graphical_browser.py))**:
   - Implements a parallel Flask web server designed specifically to serve an interactive network topology.
   - Opens the database defined by `--dbconfig`, translates discovery relationships into the topology API, and uses `--directory` to resolve evidence files on disk.
...
4. **Graphical Browser ([graphical_browser.py](graphical_browser.py))**:
   - Implements a parallel Flask web server designed specifically to serve an interactive network topology (defaults to Port `5001`).
   - Translates `devices` and `device_neighbors` relationships natively from the DB into a unified `/api/.../topology` mapping.
   - Leverages `vis-network` context in the frontend to physically render routers, switches, and firewalls as distinct FontAwesome badge nodes colored dynamically by Vendor.
   - Includes a native glassmorphic JS modal: Clicking on any node issues an asynchronous background call to `/api/.../device/...` to pop up the same tabular system details/interfaces/config evidence in real time, without leaving the physics canvas.
5. **Documentation**:
...
- Booted [graphical_browser.py](graphical_browser.py) locally and directly fetched the generated topology JSON trees, proving valid schema transmission to the `vis.js` client.
- The web engines correctly formatted the Jinja blocks. The device views successfully aggregated multiple scans and correctly read the config evidence text files from disk to present to the user.
