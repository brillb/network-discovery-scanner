...
When we executed `python scanner_orchestrator.py --targets dummy_targets.csv --keys dummy_keys.yaml --ssh-commands ssh_commands.yaml --dbconfig db.yaml`, the following pipeline occurred:

1. **Database Initialization**: The orchestrator created a new timestamped folder (e.g., `discovered_device_evidence_20260311_004438`) and initialized the database defined in `db.yaml` with the proper tables (`devices`, `device_inventory`, etc.).
2. **Dispatch**: The script read `127.0.0.1` and dispatched `process_single_ip.py`.
